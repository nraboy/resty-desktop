...
Portal-rendered hover tooltip built on `@floating-ui/react` (`useFloating` + `autoUpdate` +
`useHover`/`useFocus`/`useDismiss`/`useRole`), styled as `ContextMenu.tsx`'s sibling (same
surface: `bg-gray-900`/`border-gray-700`/`rounded-lg`/`shadow-xl`) so the two floating
surfaces read as one system. Not hoverable-into (no `safePolygon`) and never scrollable —
callers cap long content themselves (`lib/format.ts`'s `capList`) rather than relying on the
tooltip to make an unbounded list usable.

**When to use this vs. native `title`:** label tooltips — icon buttons, anything just naming
a control (`ActionButton.tsx`, most row actions, most of `ActivityPanel.tsx`) — stay on native
`title`; they're short, exist for accessibility, and the ~1s native delay doesn't matter.
*Content* tooltips — anything carrying real information a plain unstyled, unformatted,
undiscoverable box can't render well (multi-line lists, monospace text, a derived
percentage) — use `Tooltip`. The trigger for a content tooltip should visibly say "hover me":
`cursor-help` (never `cursor-default`, which claims the opposite) plus a dotted underline
(`underline decoration-dotted underline-offset-2`) inheriting the surrounding text color
rather than a hardcoded `decoration-*` shade. Converted sites: SnapshotsPage's paths-count
cell (first 5 full paths + "+N more", monospace — see that entry), RepositoriesPage's
raw/stored-size headline (only wrapped when `formatRepoSize`'s `tooltip` is present),
ActivityPanel's Upcoming Tasks plan-name list (one name per line, capped via `capList`), and
SettingsPage's "Failures" notification label (only the word itself gets the underline/tooltip,
since the enclosing `<label>` is itself a click target for the checkbox and keeps
`cursor-pointer`). This split is deliberate, not partial completion — see `docs/decisions.md`.
...
formatBytes, formatSize, formatDate, formatDateOnly, formatTimestamp, formatDuration; capList(items, limit) caps a list for bounded tooltip display — `{ shown, remaining }` — used by SnapshotsPage's paths tooltip and ActivityPanel's plan-name tooltip so neither renders an unbounded list
